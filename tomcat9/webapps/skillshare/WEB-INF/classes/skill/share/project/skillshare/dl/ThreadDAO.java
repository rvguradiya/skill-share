...
ResultSet resultSet=statement.executeQuery("select * from thread ORDER BY created_at DESC");
...
//thread.setTime(resultSet.getTimestamp("created_at"));

thread.setCode(resultSet.getInt(1));
thread.setSubject(resultSet.getString(2).trim());
thread.setHeading(resultSet.getString(3).trim());
thread.setContent(resultSet.getString(4).trim());
thread.setLink(resultSet.getString(5).trim());
thread.setImagePath(resultSet.getString(6).trim());
thread.setType(resultSet.getString(7).trim());
thread.setUserId(resultSet.getInt(9));
thread.setTime(resultSet.getTimestamp(8));
...
public ArrayList<ThreadDTOInterface> searchThreads(String searchString) throws DAOException
{
ArrayList<ThreadDTOInterface> threads=new ArrayList<ThreadDTOInterface>();
try
{
Connection connection=DAOConnection.getConnection();
PreparedStatement preparedStatement=connection.prepareStatement("select * from thread where (content like ? or heading like ?) ORDER BY created_at DESC");
